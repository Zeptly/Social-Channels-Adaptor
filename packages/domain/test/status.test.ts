...
import { aggregatePostStatus, SocialError } from "../src/index.js";
...
describe("SocialError", () => {
  it("serializes to the canonical error body", () => {
    const e = new SocialError("CAPABILITY_NOT_SUPPORTED", "nope", { details: { network: "facebook" } });
    ...
    expect(new SocialError("PROVIDER_RATE_LIMITED", "x").retryable).toBe(true);
  });
});
