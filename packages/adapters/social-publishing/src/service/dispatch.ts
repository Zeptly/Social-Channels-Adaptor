...
import { getConnectionsForWorkspace, recordAudit, recordProviderEvent, safeMessage, type SystemActor, toGatewayError } from "@zeptly-gateway/gateway-core";
import { aggregatePostStatus, type PostStatus, type SocialNetwork, type TargetStatus } from "../contract/index.js";
import type { RemotePostState } from "../port.js";
...
/** Largest instant PostgreSQL accepts from an ISO string (5-digit years like "+275760" are rejected). */
const NO_HORIZON = new Date("9999-12-31T23:59:59.999Z");

/** Latest publish instant that may be handed to the provider now (no horizon → everything is eligible). */
export function handoffHorizonEnd(now: Date, providerHorizonMs: number | undefined, marginMs: number): Date {
  if (providerHorizonMs === undefined) return NO_HORIZON;
...
export async function claimPublications(
  ...
  opts: { workerId: string; limit: number; ids?: string[] },
): Promise<SocialPublicationRow[]> {
  const now = ctx.now();
  const provider = ctx.publishing;
  const horizon = handoffHorizonEnd(now, provider.schedulingHorizonMs, ctx.publishingSettings.handoffMarginMs);
...
export async function dispatchClaimed(ctx: SocialPublishingContext, pub: SocialPublicationRow, workerId: string): Promise<DispatchOutcome> {
  ...
  try {
    ...
    if (eligible.length === 0) {
      return await finalizeFailure(ctx, pub, workerId, new GatewayError("CONNECTION_NOT_ACTIVE", "No active connection remains for this publication"), { terminal: true });
    }
    const now = ctx.now();
...
export function settleTarget(
  ...
  remote: RemotePostState["targets"][number] | undefined,
...
export async function applyRemoteState(
  ...
  const outcome = await ctx.db.transaction(async (tx) => {
    ...
    if (status !== pub.status) {
      await recordAudit(tx, opts.source === "webhook" ? { service: `webhook:${pub.provider}`, requestId: pub.id } : worker, {
...
async function finalizeFailure(ctx: SocialPublishingContext, pub: SocialPublicationRow, workerId: string, err: unknown, opts: { terminal?: boolean } = {}): Promise<DispatchOutcome> {
  const now = ctx.now();
  const se = toGatewayError(err, "PUBLICATION_FAILED");
  const pe = err instanceof UpstreamError ? err : undefined;
...
export async function runHandoff(ctx: SocialPublishingContext, workerId: string, opts: { limit?: number; ids?: string[] } = {}): Promise<DispatchOutcome[]> {
  const outcomes: DispatchOutcome[] = [];
  const claimed = await claimPublications(ctx, { workerId, limit: opts.limit ?? 25, ...(opts.ids ? { ids: opts.ids } : {}) });
