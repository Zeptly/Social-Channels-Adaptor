...
import { type Executor, type SocialMediaRow, socialMedia, type Workspace } from "@zeptly-gateway/database";
import { type Actor, assertUuid, enqueueJob, recordAudit, safeMessage, toGatewayError } from "@zeptly-gateway/gateway-core";
import { and, eq, inArray } from "drizzle-orm";
import type { MediaKind, RegisterMediaRequest, SocialMedia } from "../contract/index.js";
import type { RemoteMedia } from "../port.js";
...
export type MediaPolicy = Record<MediaKind, { mimeTypes: Set<string>; maxBytes: number }>;

const policies = new WeakMap<NetworkCatalog, MediaPolicy>();

/** Union of mime types / size limits across the gateway's networks (per-network checks happen at post validation). */
export function mediaPolicy(catalog: NetworkCatalog): MediaPolicy {
  const cached = policies.get(catalog);
  if (cached) return cached;
  const policy = { image: { mimeTypes: new Set<string>(), maxBytes: 0 }, video: { mimeTypes: new Set<string>(), maxBytes: 0 } };
  for (const d of catalog.networks()) {
  ...
  policies.set(catalog, policy);
  return policy;
}

export function mediaKindFor(MEDIA_POLICY: MediaPolicy, contentType: string): MediaKind {
  const ct = contentType.toLowerCase();
  if (MEDIA_POLICY.image.mimeTypes.has(ct)) return "image";
  if (MEDIA_POLICY.video.mimeTypes.has(ct)) return "video";
  throw new GatewayError("MEDIA_INVALID", "Unsupported media content type", { details: { contentType } });
}

/** Media is reusable by the provider until shortly before its provider-side expiry. */
const EXPIRY_SAFETY_MS = 60 * 60_000;

export async function registerMedia(ctx: SocialPublishingContext, actor: Actor, req: RegisterMediaRequest): Promise<SocialMedia> {
  const MEDIA_POLICY = mediaPolicy(ctx.socialCatalog);
  const contentType = req.contentType.toLowerCase();
  const kind = mediaKindFor(MEDIA_POLICY, contentType);
  if (req.sizeBytes && req.sizeBytes > MEDIA_POLICY[kind].maxBytes) {
    throw new GatewayError("MEDIA_INVALID", "Media exceeds the maximum supported size", { details: { maxBytes: MEDIA_POLICY[kind].maxBytes } });
  }
  const ws = actor.workspace;
  // Media is registered once with this gateway's provider and reused by any target.
  if (ctx.socialCatalog.networksWith("media").length === 0) throw new GatewayError("CAPABILITY_NOT_SUPPORTED", "This gateway does not support media");
  ...
  if (req.source.type === "upload") {
    ...
    const [row] = await ctx.db
      ...
      .returning();
    if (!row) throw new GatewayError("INTERNAL_ERROR", "Media registration failed");
  ...
  await assertPublicHttpsUrl(source.url, { skipDns: ctx.publishingSettings.skipMediaDnsCheck ?? false });
  const row = await ctx.db.transaction(async (tx) => {
    const [r] = await tx
      ...
      .returning();
    if (!r) throw new GatewayError("INTERNAL_ERROR", "Media registration failed");
...
export async function processMediaUpload(ctx: SocialPublishingContext, mediaId: string): Promise<void> {
  ...
  try {
    ...
    const se = toGatewayError(err, "PROVIDER_UNAVAILABLE");
...
async function uploadFromSource(ctx: SocialPublishingContext, row: SocialMediaRow): Promise<RemoteMedia> {
  if (!row.sourceUrl) throw new GatewayError("MEDIA_INVALID", "Media has no durable source URL");
  await assertPublicHttpsUrl(row.sourceUrl, { skipDns: ctx.publishingSettings.skipMediaDnsCheck ?? false });
  return ctx.publishing.uploadFromUrl({
    sourceUrl: row.sourceUrl,
    filename: row.filename,
    contentType: row.contentType,
    maxBytes: mediaPolicy(ctx.socialCatalog)[row.kind as MediaKind].maxBytes,
  });
}

export async function completeMediaUpload(ctx: SocialPublishingContext, actor: Actor, id: string, req: { sizeBytes?: number }): Promise<SocialMedia> {
  const row = await loadMedia(ctx.db, actor.workspace, id);
  if (row.status === "ready") return toMedia(row, actor.workspace);
  if (row.status !== "pending_upload" || !row.providerMediaId) throw new GatewayError("INVALID_STATE", "Media is not awaiting an upload");
  const provider = ctx.publishing;
  let pm: RemoteMedia;
  try {
    ...
    if (se.code === "PROVIDER_REJECTED") {
      ...
      throw new GatewayError("MEDIA_INVALID", "The provider did not accept the uploaded file", { details: se.details ?? {} });
    }
    throw se;
  }
...
async function loadMedia(db: Executor, ws: Workspace, id: string): Promise<SocialMediaRow> {
  ...
  if (!rows[0]) throw new GatewayError("MEDIA_NOT_FOUND", "Media not found");
  return rows[0];
}

export async function getMedia(ctx: SocialPublishingContext, actor: Actor, id: string): Promise<SocialMedia> {
...
export async function loadWorkspaceMedia(db: Executor, workspaceId: string, ids: string[]): Promise<Map<string, SocialMediaRow>> {
  ...
  if (missing.length) throw new GatewayError("MEDIA_NOT_FOUND", "Media not found", { details: { mediaIds: missing } });
  return map;
}
 ...
 */
export async function resolveProviderMedia(ctx: SocialPublishingContext, workspaceId: string, ids: string[], publishAt: Date): Promise<RemoteMedia[]> {
  ...
  for (const id of ids) {
    ...
    if (row.status !== "ready" || !row.providerMediaId || !row.providerUrl) {
      throw new GatewayError("MEDIA_INVALID", "Media is not ready", { details: { mediaId: id, status: row.status } });
    }
    ...
    if (row.providerExpiresAt && row.providerExpiresAt.getTime() < deadline) {
      if (!row.sourceUrl) {
        throw new GatewayError("MEDIA_INVALID", "Uploaded media expires at the provider before the publish time; register it by URL instead", { details: { mediaId: id } });
      }
      const pm = await uploadFromSource(ctx, row);
      await ctx.db
        .update(socialMedia)
