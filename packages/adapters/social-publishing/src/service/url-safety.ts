...
export async function assertPublicHttpsUrl(raw: string, opts: { skipDns?: boolean } = {}): Promise<URL> {
  ...
  try {
    ...
    throw new GatewayError("MEDIA_INVALID", "Media URL is not a valid URL");
  }
  if (url.protocol !== "https:") throw new GatewayError("MEDIA_INVALID", "Media URL must use https");
  if (url.username || url.password) throw new GatewayError("MEDIA_INVALID", "Media URL must not embed credentials");
  const host = url.hostname.replace(/^\[|\]$/g, "");
  if (host === "localhost" || host.endsWith(".localhost") || host.endsWith(".internal") || host.endsWith(".local")) {
    throw new GatewayError("MEDIA_INVALID", "Media URL host is not public");
  }
  if (isIP(host)) {
    if (!isPublicAddress(host)) throw new GatewayError("MEDIA_INVALID", "Media URL host is not public");
    return url;
  }
  ...
  try {
    ...
    throw new GatewayError("MEDIA_INVALID", "Media URL host does not resolve");
  }
