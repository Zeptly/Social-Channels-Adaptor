...
import { requestHash, toGatewayError } from "@zeptly-gateway/gateway-core";
import { redact, redactString, registerSecret } from "@zeptly-gateway/observability";
import { describe, expect, it } from "vitest";
import { OUTSTAND_SOCIAL_NETWORKS as OUTSTAND_NETWORKS } from "../src/outstand/index.js";
import { aggregatePublicationStatus, backoffFor, handoffHorizonEnd, settleTarget } from "../src/service/dispatch.js";
import { assertPublicHttpsUrl, isPublicAddress } from "../src/service/url-safety.js";
import { validateTargetContent } from "../src/service/validation.js";
...
describe("provider error translation", () => {
  const pe = (kind: UpstreamError["kind"], status?: number) => new UpstreamError("outstand", kind, "Outstand POST /posts/ → 429: slow", { retryable: kind === "rate_limit", ambiguous: false, ...(status ? { status } : {}), retryAfterSeconds: 30 });
  it("maps provider kinds to canonical codes with sanitized diagnostics", () => {
    expect(toGatewayError(pe("rate_limit", 429))).toMatchObject({ code: "PROVIDER_RATE_LIMITED", retryable: true, details: { retryAfterSeconds: 30 } });
    expect(toGatewayError(pe("validation", 400)).code).toBe("PROVIDER_REJECTED");
    expect(toGatewayError(pe("auth", 401)).code).toBe("PROVIDER_UNAVAILABLE");
    expect(toGatewayError(pe("unsupported")).code).toBe("CAPABILITY_NOT_SUPPORTED");
    expect(toGatewayError(new Error("boom")).code).toBe("INTERNAL_ERROR");
  });
});
