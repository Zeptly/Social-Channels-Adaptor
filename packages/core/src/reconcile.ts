import { socialPostTargets, type SocialPublicationRow, socialPublications } from "@zeptly-social/database";
 ...
 */
export async function reconcilePublication(ctx: ServiceContext, publicationId: string, source: "webhook" | "reconcile" = "reconcile"): Promise<string> {
  ...
  try {
    ...
    if (err instanceof ProviderError && err.kind === "not_found") return markVanished(ctx, pub);
    throw err;
  }
}

/** The provider no longer knows the post: non-terminal targets fail explicitly. */
async function markVanished(ctx: ServiceContext, pub: SocialPublicationRow): Promise<string> {
  const now = ctx.now();
 ...
 */
export async function enqueueDueReconciliations(ctx: ServiceContext, limit = 100): Promise<number> {
  const now = ctx.now();
