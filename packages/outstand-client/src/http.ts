...
import { OUTSTAND, OutstandError } from "./errors.js";
 ...
 *  - Errors become OutstandError with a redacted, bounded message.
 */
export class OutstandHttp {
  ...
  async request(path: string, opts: RequestOptions = {}): Promise<unknown> {
    ...
    let lastErr: OutstandError | undefined;
    for (let attempt = 1; attempt <= attempts; attempt++) {
      try {
        return await this.once(path, method, opts, requestId, attempt);
      } catch (err) {
        if (!(err instanceof OutstandError)) throw err;
        lastErr = err;
    ...
    throw lastErr ?? new OutstandError("network", "request failed", { retryable: true, ambiguous: Boolean(opts.mutating) });
  }
  ...
  private async once(path: string, method: string, opts: RequestOptions, requestId: string, attempt: number): Promise<unknown> {
    ...
    try {
      ...
      throw new OutstandError(isTimeout ? "timeout" : "network", `Outstand ${method} ${logPath} ${isTimeout ? "timed out" : "network error"}`, {
        retryable: true,
    ...
    if (json === undefined && text) {
      throw new OutstandError("protocol", `Outstand ${method} ${logPath} returned non-JSON`, {
        status: res.status,
        retryable: false,
        ambiguous: Boolean(opts.mutating),
      });
    ...
    if (env.success && env.data.success === false) {
      throw new OutstandError("validation", `Outstand ${method} ${logPath} failed: ${errorMessage(json)}`, {
        status: res.status,
        retryable: false,
        ambiguous: false,
  ...
  private httpError(res: Response, method: string, path: string, json: unknown, text: string, mutating: boolean): OutstandError {
    ...
    if (status === 401 || status === 403) return new OutstandError("auth", base, { status, retryable: false, ambiguous: false, details });
    if (status === 404) return new OutstandError("not_found", base, { status, retryable: false, ambiguous: false, details });
    if (status === 409) {
      // Same Idempotency-Key still in flight, or reused with a different body.
      return new OutstandError("conflict", base, { status, retryable: true, ambiguous: true, retryAfterSeconds, details });
    }
    if (status === 429) return new OutstandError("rate_limit", base, { status, retryable: true, ambiguous: false, retryAfterSeconds, details });
    if (status >= 500) return new OutstandError("server", base, { status, retryable: true, ambiguous: mutating, retryAfterSeconds, details });
    return new OutstandError("validation", base, { status, retryable: false, ambiguous: false, details });
  }
}
