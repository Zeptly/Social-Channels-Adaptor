/**
 * Outstand wire shapes ↔ typed client results. Pure functions, no I/O.
 * PRIVATE to the outstand-client package: never exported from index.ts.
 *
...
export function mapAccount(raw: unknown): OutstandAccount {
...
export function mapPending(raw: unknown): OutstandPendingConnection {
...
export function mapAccountList(data: unknown): OutstandAccount[] {
...
export function mapMedia(raw: unknown, fallbackFilename: string): OutstandMedia {
...
export function mapTargetStatus(s: string | undefined): OutstandTargetStatus {
...
export function mapPost(raw: unknown): OutstandPostState {
...
export function networkOptions(network: string, options: Record<string, unknown>): Partial<OutstandCreatePostBody> {
  switch (network) {
    case "threads": {
...
export function buildCreatePostBody(input: OutstandCreatePostInput): OutstandCreatePostBody {
...
export function mapMessage(raw: unknown, fallbackConversationId?: string): OutstandMessage {
  const m = wireMessageSchema.parse(raw);
  const dir = (m.direction ?? "").toLowerCase();
  const direction: OutstandMessage["direction"] =
    dir === "outbound" || dir === "sent" || m.is_from_business === true ? "outbound" : "inbound";
  const s = (m.status ?? "").toLowerCase();
  const status: OutstandMessage["status"] =
...
export function mapConversation(raw: unknown, fallbackAccountId?: string): OutstandConversation {
