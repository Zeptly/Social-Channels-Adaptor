...
import type { OutstandWebhookEvent, ParsedOutstandWebhook } from "./types.js";
import { z } from "zod";
...
function protocol(message: string): OutstandError {
  return new OutstandError("protocol", message, { retryable: false, ambiguous: false });
}

export function parseWebhook(rawBody: Buffer): ParsedOutstandWebhook {
  let json: unknown;
  try {
  ...
  if (type === "post.published" || type === "post.error") {
    ...
    const event: OutstandWebhookEvent = {
      kind: "post_outcome",
  ...
  if (type === "account.token_expired") {
    ...
    return {
      ...
      event: { kind: "account_token_expired", accountExternalId: d.data.accountId, ...(d.data.error ? { reason: d.data.error } : {}), occurredAt },
    };
  }
