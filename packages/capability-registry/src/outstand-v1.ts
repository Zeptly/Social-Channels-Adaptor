import type { NetworkDescriptor, SocialCapabilities, SocialNetwork } from "@zeptly-social/domain";

/**
 * Version-controlled capability registry for the Outstand provider (V1).
 *
 * Every flag states what THIS SERVICE exposes through the Outstand adapter, and
...
export const OUTSTAND_NETWORKS: Readonly<Record<SocialNetwork, NetworkDescriptor>> = {
  linkedin: {
    network: "linkedin",
    displayName: "LinkedIn",
    connectionStrategy: "oauth_redirect",
    supportedStrategies: ["oauth_redirect"],
    capabilities: { ...BASE },
    constraints: {
      maxTextLength: 3000,
      textRequired: true,
  ...
  bluesky: {
    network: "bluesky",
    displayName: "Bluesky",
    connectionStrategy: "provider_managed",
    supportedStrategies: ["provider_managed", "credentials"],
    capabilities: { ...BASE },
    constraints: {
      maxTextLength: 300,
      textRequired: false,
