...
import { type GatewayConnection, GatewayError, type ProvisioningSession } from "@zeptly-gateway/gateway-contract";
import {
  type Executor,
  providerAccounts,
...
import type { ChannelCatalog, ChannelDescriptor, ProviderAccountPort, ProviderAccountRecord } from "./accounts.js";
import { and, desc, eq, gt, inArray, isNull, lt, or } from "drizzle-orm";
import { recordAudit, recordProviderEvent } from "./audit.js";
import type { Actor, GatewayContext, SystemActor } from "./context.js";
...
/** Gateway infrastructure needed for provisioning and connection health. */
export interface ConnectionsContext extends GatewayContext {
...
export interface CreateConnectionInput {
  channel: string;
  returnUrl?: string;
  /** Forwarded once to the provider (credentials strategy); never persisted or logged. */
  credentials?: { handle: string; appPassword: string };
}

export function listChannels(ctx: ConnectionsContext): ChannelDescriptor[] {
  return ctx.channels.channels();
}

function assertReturnUrl(ctx: ConnectionsContext, returnUrl: string | undefined): string {
  if (!returnUrl) throw new GatewayError("VALIDATION_ERROR", "returnUrl is required for this connection strategy");
  let origin: string;
  try {
    ...
    throw new GatewayError("VALIDATION_ERROR", "returnUrl is not a valid URL");
  }
  if (!ctx.settings.allowedReturnOrigins.includes(origin)) {
    throw new GatewayError("VALIDATION_ERROR", "returnUrl origin is not allowed", { details: { origin } });
  }
  return returnUrl;
}
...
export async function createConnection(ctx: ConnectionsContext, actor: Actor, req: CreateConnectionInput, reconnect?: GatewayConnectionRow): Promise<ConnectionResult> {
  ...
  const providerName = ctx.gatewayId;
  const provider = ctx.accounts;
  const now = ctx.now();
  const ws = actor.workspace;

  if (req.credentials) {
    if (!descriptor.supportedStrategies.includes("credentials") || !provider.connectWithCredentials) {
      throw new GatewayError("CAPABILITY_NOT_SUPPORTED", `Credential-based connection is not supported for ${network}`, { details: { network } });
    }
    const [session] = await ctx.db
      ...
      .returning();
    if (!session) throw new GatewayError("INTERNAL_ERROR", "Could not create provisioning session");
    let accounts: ProviderAccountRecord[];
    try {
      // Credentials are forwarded once and never persisted or logged.
      accounts = await provider.connectWithCredentials({ channel: network, tenantRef: ws.providerTenantRef, credentials: req.credentials });
    } catch (err) {
 ...
 */
export async function handleProviderCallback(ctx: ConnectionsContext, state: string, query: { session?: string; error?: string }): Promise<string> {
  ...
  if (!session?.returnUrl) throw new GatewayError("PROVISIONING_NOT_FOUND", "Unknown or already used provisioning link");
  ...
  try {
    ...
    if (pending.channel !== session.network) {
    ...
    if (pending.options.length <= 1) {
      ...
      if (!ws) throw new GatewayError("INTERNAL_ERROR", "workspace missing");
...
async function loadSession(ctx: ConnectionsContext, ws: Workspace, id: string): Promise<ProvisioningSessionRow> {
  ...
  const rows = await ctx.db
    ...
    .limit(1);
  if (!rows[0]) throw new GatewayError("PROVISIONING_NOT_FOUND", "Provisioning session not found");
  return rows[0];
}

export async function getProvisioning(ctx: ConnectionsContext, actor: Actor, id: string): Promise<ProvisioningSession> {
...
export async function finalizeProvisioning(ctx: ConnectionsContext, actor: Actor, id: string, req: { optionIds: string[] }): Promise<ConnectionResult> {
  const session = await loadSession(ctx, actor.workspace, id);
  if (session.status === "completed") {
    const conns = session.connectionIds.length
      ? await ctx.db.select().from(gatewayConnections).where(and(inArray(gatewayConnections.id, session.connectionIds), eq(gatewayConnections.workspaceId, actor.workspace.id)))
      : [];
    return { provisioning: toProvisioning(session, actor.workspace), connections: conns.map((c) => toConnection(c, actor.workspace)) };
  }
  if (session.status !== "awaiting_selection" || !session.providerSessionToken) {
    throw new GatewayError("INVALID_STATE", "Provisioning session is not awaiting selection", { details: { status: session.status } });
  }
  if (session.expiresAt < ctx.now()) {
    ...
    throw new GatewayError("PROVISIONING_EXPIRED", "Provisioning session expired; start a new connection");
  }
  const allowed = new Set((session.options ?? []).map((o) => String(o.id)));
  const unknown = req.optionIds.filter((o) => !allowed.has(o));
  if (unknown.length && allowed.size > 0) throw new GatewayError("VALIDATION_ERROR", "Unknown option ids", { details: { optionIds: unknown } });
  const provider = ctx.accounts;
  let accounts: ProviderAccountRecord[];
  try {
 ...
 */
async function completeWithAccounts(ctx: ConnectionsContext, actor: Actor, session: ProvisioningSessionRow, accounts: ProviderAccountRecord[]): Promise<ConnectionResult> {
  const ws = actor.workspace;
  const now = ctx.now();
  const matching = accounts.filter((a) => a.channel === session.network);
  const result = await ctx.db.transaction(async (tx) => {
    const connections: GatewayConnectionRow[] = [];
    const conflicts: string[] = [];
    for (const a of matching) {
      const adopted = await adoptAccount(tx, ws, session.provider, session.network, a, now);
  ...
  if (result.connections.length === 0) {
    throw new GatewayError(result.conflicts.length ? "CONNECTION_OWNERSHIP_CONFLICT" : "PROVIDER_REJECTED", result.session.errorMessage ?? "Provisioning failed", {
      details: { provisioningId: session.id },
    });
  }
  return {
    provisioning: toProvisioning(result.session, ws),
    connections: result.connections.map((c) => toConnection(c, ws)),
  };
}

async function adoptAccount(tx: Executor, ws: Workspace, provider: string, network: string, a: ProviderAccountRecord, now: Date): Promise<GatewayConnectionRow | "conflict"> {
  const profile = {
  ...
  if (mapping) {
    ...
    const [conn] = await tx
      ...
      .where(and(eq(gatewayConnections.id, mapping.connectionId), eq(gatewayConnections.workspaceId, ws.id)))
      .returning();
    if (!conn) throw new GatewayError("INTERNAL_ERROR", "Connection mapping inconsistent");
    return conn;
  }
  const [conn] = await tx
    ...
    .returning();
  if (!conn) throw new GatewayError("INTERNAL_ERROR", "Could not create connection");
  ...
  if (inserted.length === 0) {
    ...
    await tx.delete(gatewayConnections).where(eq(gatewayConnections.id, conn.id));
    ...
    const [c] = await tx.select().from(gatewayConnections).where(eq(gatewayConnections.id, again[0].connectionId));
    return c ?? "conflict";
  }
  return conn;
}

export async function listConnections(ctx: ConnectionsContext, actor: Actor, filter: { channel?: string; status?: string } = {}): Promise<GatewayConnection[]> {
  const conds = [eq(gatewayConnections.workspaceId, actor.workspace.id)];
  if (filter.channel) conds.push(eq(gatewayConnections.network, filter.channel));
  if (filter.status) conds.push(eq(gatewayConnections.status, filter.status));
  const rows = await ctx.db
    .select()
    .from(gatewayConnections)
    .where(and(...conds))
    .orderBy(desc(gatewayConnections.createdAt));
  return rows.map((r) => toConnection(r, actor.workspace));
}

export async function getConnection(ctx: ConnectionsContext, actor: Actor, id: string): Promise<GatewayConnection> {
  assertUuid(id, "CONNECTION_NOT_FOUND");
  const { connection } = await getConnectionForWorkspace(ctx.db, actor.workspace.id, id);
  return toConnection(connection, actor.workspace);
}
...
export async function disconnectConnection(ctx: ConnectionsContext, actor: Actor, id: string): Promise<GatewayConnection> {
  assertUuid(id, "CONNECTION_NOT_FOUND");
  const { connection, account } = await getConnectionForWorkspace(ctx.db, actor.workspace.id, id);
  if (connection.status === "disconnected") return toConnection(connection, actor.workspace);
  const provider = ctx.accounts;
  try {
  ...
  const updated = await ctx.db.transaction(async (tx) => {
    ...
    const [c] = await tx
      ...
      .where(and(eq(gatewayConnections.id, connection.id), eq(gatewayConnections.workspaceId, actor.workspace.id)))
      .returning();
  ...
  return toConnection(updated, actor.workspace);
}
...
export async function markReauthorizationRequired(
  ...
  const updated = await db
    ...
    .where(and(eq(gatewayConnections.id, connection.id), eq(gatewayConnections.workspaceId, connection.workspaceId)))
 ...
 */
export async function reconcileConnections(ctx: ConnectionsContext, actor: Actor | SystemActor, workspace?: Workspace): Promise<ConnectionReconcileSummary> {
  const summary: ConnectionReconcileSummary = { checked: 0, changed: 0, adopted: 0 };
  {
    const providerName = ctx.gatewayId;
    const provider = ctx.accounts;
    let remote: ProviderAccountRecord[];
    try {
    ...
    const mapped = await ctx.db
      ...
      .innerJoin(gatewayConnections, eq(gatewayConnections.id, providerAccounts.connectionId))
      .where(and(...conds));
    ...
    for (const { connection, account } of mapped) {
      ...
      if (!r && workspace && account.tenantRef !== workspace.providerTenantRef) {
        await ctx.db.update(gatewayConnections).set({ lastCheckedAt: now }).where(eq(gatewayConnections.id, connection.id));
        continue;
      }
    ...
    for (const ws of owners) {
      ...
      for (const a of unmapped.filter((x) => x.tenantRef === ws.providerTenantRef)) {
        const session = recent.find((s) => s.network === a.channel);
        if (!session) continue;
        const res = await ctx.db.transaction(async (tx) => adoptAccount(tx, ws, providerName, a.channel, a, now));
        if (res !== "conflict") {
          summary.adopted++;
          await ctx.db.transaction(async (tx) => {
            if (session.status !== "completed") {
